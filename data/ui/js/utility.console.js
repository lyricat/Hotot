...
function interpret (cmd) {
    ...
    if (result == -1) {
        try {
            result = eval(cmd);
            result = utility.Console.form_result('out', result);
        } catch (e) {
            result = utility.Console.form_result('err', e.toString());
        }
    } else {
        result = utility.Console.form_result('out', result);
    }
...
function form_result (type, raw_result) {
    var ps = type == 'in'? utility.Console.ps_in:
        (type == 'out'? utility.Console.ps_out: utility.Console.ps_err);
    var result = '<span class="console_ps_' + type + '">'
        + ps
        + '</span>'
        + raw_result + '<br/>';
    result = result.replace(/{%COUNT%}/g, 
        '[<strong>' + utility.Console.cmd_count + '</strong>]');
    return result;
},
